...
Status: M0 completed; M1 implemented; M2 durable range-scheduling foundation implemented; M3 Parts 1-15 typed idempotent Telegram normalization, local archive storage, image normalization/fingerprinting, generic range-history execution orchestration, a real-time media-group aggregation buffer, a typed bounded Telethon history reader, source reconciliation-cursor storage, source edit/deletion lifecycle persistence, image-asset READY metadata transitions, a protected-content-respecting Telethon media-download adapter, durable image-to-source-message references, a lease-oriented image archive application workflow, PostgreSQL image archive lease/wake-up routing, and an application-only deterministic video frame-sampling boundary implemented. The repository is verified with unit tests plus offline Alembic PostgreSQL SQL rendering on 2026-09-04. Live PostgreSQL adapter verification remains pending.
...
### M3 - Telegram ingestion and media archive (Parts 1-13: typed ingestion, local immutable storage, image normalization/fingerprinting, history and realtime ingestion foundations, source lifecycle persistence, protected media download, source-message references, image archive workflow, and PostgreSQL image-archive leases implemented)
...
- `PillowImageProcessor` implements the `ImageProcessor` port: it applies EXIF orientation, preserves alpha when present, bounds dimensions, emits metadata-free WebP, computes SHA-256 values for both source and archive bytes, and calculates a deterministic 64-bit DCT pHash from normalized display pixels. Ingestion now creates a pending `image_assets` row for every image media asset. `ImageArchiveService` still keeps archive I/O outside a database transaction: it publishes the immutable WebP first, verifies its stored size, and only then calls the narrow metadata port to mark the asset READY. The PostgreSQL transition permits a replay only when its immutable backend/key/content facts agree; deleted, missing, and conflicting rows remain non-ready.
...
- M3 Part 15 adds a `VideoFrameExtractor` port and `VideoFrameSamplingService` application boundary. Given injected decoded stills and the existing image processor, it validates the source bytes/duration, evenly spreads a bounded candidate set, chooses a zero-second cover when configured, and retains chronological representative frames only when their timestamps and normalized 64-bit pHashes are sufficiently distinct. Its immutable result validates cover/representative ordering and duration bounds for later persistence. This part intentionally adds no FFmpeg adapter, video database tables, storage calls, durable queue/worker, or inference behavior.
...
- `TelegramMediaDownloadRequest` carries a validated source channel UUID, Telegram message ID, and durable source-asset ID. `TelethonMediaDownloader` resolves that source peer, loads the exact message, returns only non-empty in-memory media bytes, rejects missing media, and translates `noforwards` or Telethon restricted-forward failures to `TelegramProtectedContentError`. It deliberately does not bypass Telegram content protection. Telethon-mapped image assets retain their originating positive Telegram message ID in the domain DTO and image_assets, including album components. `ImageArchiveWorker` supplies provider-neutral orchestration over an injected claim repository: no long transaction spans Telegram or archive I/O; source deletion/protected/unavailable outcomes become stable terminal reasons; unexpected download/archive problems release the claim for retry; and READY metadata follows verified storage publication. The PostgreSQL claim-state adapter claims only pending source images through `FOR UPDATE SKIP LOCKED`, persists a lease token and expiry, permits expired-lease recovery, and allows READY, terminal failure, or retry release only for the matching lease. The ingestion transaction now creates a pending `image_archive` durable wake-up for each pending image asset; the Celery transport routes only its normalized UUID; and terminal READY/deleted/failed states complete the wake-up. An unconfigured worker runtime safely leaves the durable wake-up unfinished rather than pretending that external Telegram/storage work occurred.
...
- Production composition of the archive worker around a real Telethon identity/session, `TelethonMediaDownloader`, Pillow processor, and configured local/object storage; a concrete FFmpeg (or equivalent) video-frame extractor plus durable video asset/frame persistence, storage publication, queue/worker routing; and protected-content handling beyond the download boundary. The deterministic application-only video sampling policy is complete, but it has no external-I/O or PostgreSQL composition yet. PostgreSQL durable wake-up creation/repair and UUID-only Celery routing are implemented, but no live worker process currently provides the external Telegram/archive dependency bundle.
- Live PostgreSQL adapter verification for concurrent image-archive lease claims/reclamation and replay/upsert behavior. SQLite is not a substitute because the schema relies on PostgreSQL row locking, partial indexes, and conflict semantics.
