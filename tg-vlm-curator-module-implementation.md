...
- `TelegramMediaDownloadRequest` carries a validated source channel UUID, Telegram message ID, and durable source-asset ID. `TelethonMediaDownloader` resolves that source peer, loads the exact message, returns only non-empty in-memory media bytes, rejects missing media, and translates `noforwards` or Telethon restricted-forward failures to `TelegramProtectedContentError`. It deliberately does not bypass Telegram content protection and does not yet compose download/normalization/archive work into a durable worker. Telethon-mapped image assets retain their originating positive Telegram message ID in the domain DTO and image_assets, including album components, which supplies the exact durable lookup needed by that future worker. ImageArchiveWorker supplies the provider-neutral orchestration over an injected claim repository: no long transaction spans Telegram or archive I/O; source deletion/protected/unavailable outcomes become stable terminal reasons; unexpected download/archive problems release the claim for retry; and READY metadata follows verified storage publication. The PostgreSQL claim-state adapter is now implemented: a short transaction claims only pending source images through `FOR UPDATE SKIP LOCKED`, persists a lease token and expiry, permits expired-lease recovery, and allows READY, terminal failure, or retry release only for the matching lease. Deployable queue/task composition is deliberately still pending.
...
- Durable archive-worker queue/task composition around the Telegram media downloader; video cover/representative-frame extraction; and protected-content handling beyond the download boundary. The PostgreSQL lease repository, image-asset metadata, standalone downloader, and application workflow are implemented, but no live worker process composes the full download/normalize/archive lifecycle yet.
