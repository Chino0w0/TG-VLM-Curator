...
class ProcessingRange:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.boundary_kind, BoundaryKind):
            raise DomainValidationError("boundary_kind must be a BoundaryKind")
...
class RangeExecution:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.range_id, str) or not self.range_id.strip():
            raise DomainValidationError("range_id must not be blank")
        if not isinstance(self.status, RangeExecutionStatus):
            raise DomainValidationError("status must be a RangeExecutionStatus")
