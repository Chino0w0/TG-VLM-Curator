...
class ProcessingRangeTests(unittest.TestCase):
    ...
    def test_range_boundaries_require_typed_aware_values(self) -> None:
        with self.assertRaises(DomainValidationError):
            ProcessingRange(
                "range-1",
        ...
        with self.assertRaises(DomainValidationError):
            ProcessingRange(
                "range-1", START, "unsupported", latest_quiet_period=timedelta(minutes=5)
            )
        with self.assertRaises(DomainValidationError):
            latest_boundary_is_stable(START, END, timedelta(0))
