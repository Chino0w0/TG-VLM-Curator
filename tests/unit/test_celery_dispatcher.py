...
class CeleryTaskDispatcherTests(unittest.TestCase):
    ...
    def test_dispatches_only_a_normalized_video_asset_uuid(self) -> None:
        celery = FakeCelery()
        dispatcher = CeleryTaskDispatcher(celery)
        video_asset_id = str(uuid4()).upper()

        asyncio.run(dispatcher.dispatch(queue=VIDEO_ARCHIVE_QUEUE, entity_id=video_asset_id))
    ...
    def test_client_declares_all_durable_worker_queues(self) -> None:
