...
class FakeMessageIngestRepository(TelegramMessageIngestRepository):
    ...
    async def upsert_message(self, *, message: NormalizedTelegramMessage) -> None:
        ...
        identity = (
            ...
            message.telegram_grouped_id or message.primary_telegram_message_id,
        )
        existing = self.messages.get(identity)
        if existing is None:
            self.messages[identity] = message
            return
        parts_by_id = {part.telegram_message_id: part for part in existing.parts}
        parts_by_id.update({part.telegram_message_id: part for part in message.parts})
        self.messages[identity] = normalized_from_parts(
            source_channel_id=message.source_channel_id,
            telegram_grouped_id=message.telegram_grouped_id,
            parts=parts_by_id.values(),
        )
...
class TelegramNormalizationTests(unittest.TestCase):
    def test_album_uses_group_identity_and_actual_sorted_component_ids(self) -> None:
        ...
        message = album[0]
        self.assertEqual(message.telegram_grouped_id, 701)
        self.assertEqual(message.primary_telegram_message_id, 101)
        self.assertEqual(message.telegram_message_ids, (101, 103))
        self.assertEqual(message.published_at, NOW)
        self.assertEqual(message.original_text, "album caption")
        self.assertEqual([asset.asset_id for asset in message.media], ["asset-101", "asset-103"])
    ...
    def test_same_grouped_id_from_distinct_sources_does_not_merge(self) -> None:
        ...
        normalized = normalize_telegram_messages(
            (
                TelegramMessage(first_source, 7, NOW, telegram_grouped_id=99),
                TelegramMessage(second_source, 7, NOW, telegram_grouped_id=99),
            )
        )
...
class MessageIngestServiceTests(unittest.TestCase):
    def test_history_and_update_share_an_idempotent_repository_path(self) -> None:
        ...
        incoming = TelegramMessage(
            ...
            published_at=NOW,
            original_text="same Telegram message",
        )
    ...
    def test_later_album_part_updates_the_same_logical_group(self) -> None:
        ...
        asyncio.run(
            service.ingest_history(
                (TelegramMessage(source_channel_id, 10, NOW, telegram_grouped_id=800),)
            )
        )
        asyncio.run(
        ...
        self.assertEqual(stored.telegram_grouped_id, 800)
