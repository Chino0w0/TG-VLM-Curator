...
from tgcurator.infrastructure.database import due_wakeup_claim_statement
...
class FakeWakeupRepository:
    def __init__(self, claims: tuple[ClaimedWakeup, ...]) -> None:
        ...
        self.failures: list[tuple[str, str, datetime, datetime]] = []
    ...
    async def reschedule_after_failure(
        ...
        *,
        ...
        self.failures.append((wakeup_id, lease_token, now, retry_after))
        return True
...
class DurableWakeupDispatcherTests(unittest.TestCase):
    def test_successful_dispatch_remains_repairable_until_business_completion(self) -> None:
        entity_id = str(uuid4())
    ...
    def test_broker_failure_releases_claim_for_short_retry(self) -> None:
        entity_id = str(uuid4())
        claim = ClaimedWakeup(str(uuid4()), "range_execution", entity_id, str(uuid4()))
        repository = FakeWakeupRepository((claim,))

        report = asyncio.run(
            DurableWakeupDispatcher(repository, ControlledDispatcher({entity_id})).dispatch_due(
                now=NOW
            )
        )
        ...
        self.assertEqual(
            repository.failures,
            [(claim.wakeup_id, claim.lease_token, NOW, NOW + timedelta(seconds=30))],
        )
        self.assertEqual(repository.acknowledgements, [])

    def test_rejects_invalid_limit_and_claim_query_is_nonblocking(self) -> None:
        with self.assertRaises(DomainValidationError):
            asyncio.run(
                DurableWakeupDispatcher(
                    FakeWakeupRepository(()), ControlledDispatcher()
                ).dispatch_due(now=NOW, limit=0)
            )
        sql = str(
            due_wakeup_claim_statement(now=NOW, limit=10).compile(
