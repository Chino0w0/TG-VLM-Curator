...
class ConfigurationVersionTests(unittest.TestCase):
    def test_draft_is_canonical_and_published_version_cannot_be_revised(self) -> None:
        ...
        self.assertEqual(draft.content_json, '{"enabled":true,"filters":["text"]}')

        published = draft.publish(datetime(2026, 9, 7, tzinfo=UTC))
    ...
    def test_configuration_snapshots_reject_non_object_or_non_json_values(self) -> None:
        with self.assertRaises(DomainValidationError):
            ConfigurationVersion.draft(
                version_id="bad-v1",
                configuration_kind="prompt",
