...
class ImageArchiveWorkerTests(unittest.TestCase):
    ...
    def test_records_terminal_protected_and_unavailable_sources_without_archiving(self) -> None:
        for error, error_code in (
            (TelegramProtectedContentError(), "TELEGRAM_PROTECTED_CONTENT"),
            (TelegramMediaUnavailableError(), "TELEGRAM_MEDIA_UNAVAILABLE"),
        ):
            with self.subTest(error_code=error_code):
                ...
                self.assertEqual(repository.failures[0][0], error_code)
                self.assertFalse(repository.failures[0][2])
                self.assertEqual(repository.ready, [])
                self.assertEqual(repository.completed_wakeups, [])

    def test_records_retryable_download_failure_without_persisting_exception_text(self) -> None:
        repository = _Repository(_claim())
        worker = _worker(
            repository=repository,
            downloader=_Downloader(error=RuntimeError("https://secret.invalid/token")),
            storage=_Storage(),
        )

        self.assertTrue(asyncio.run(worker.process(image_asset_id=repository.asset_id, now=NOW)))
        error_code, error_type, retryable, *_ = repository.failures[0]
        self.assertEqual(error_code, "TELEGRAM_DOWNLOAD_FAILED")
        self.assertEqual(error_type, "TelegramDownloadError")
        self.assertTrue(retryable)
        self.assertNotIn("secret", repr(repository.failures))
        self.assertEqual(repository.completed_wakeups, [])

    def test_records_retryable_archive_failure(self) -> None:
        repository = _Repository(_claim())
        storage = _Storage(size_delta=1)
        worker = _worker(
            repository=repository,
            downloader=_Downloader(content=b"source"),
            storage=storage,
        ...
        self.assertTrue(asyncio.run(worker.process(image_asset_id=repository.asset_id, now=NOW)))
        self.assertEqual(repository.failures[0][0], "IMAGE_ARCHIVE_FAILED")
        self.assertTrue(repository.failures[0][2])
        self.assertEqual(repository.ready, [])
    ...
    def test_never_downloads_a_source_message_already_recorded_as_deleted(self) -> None:
        ...
        self.assertEqual(repository.failures[0][0], "SOURCE_MESSAGE_DELETED")
        self.assertFalse(repository.failures[0][2])
